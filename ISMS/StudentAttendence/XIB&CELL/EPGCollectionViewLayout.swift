...
class EPGCollectionViewLayout: UICollectionViewLayout {
  ...
  override func prepare() {
      ...
      for section in 0...sectionCount - 1 {
        ...
        if let rowCount = collectionView?.numberOfItems(inSection: section), rowCount > 0 {
          for item in 0...rowCount - 1 {
            ...
            }else  if let _ = program  {
                xPos = (cellWidth * (program!.schedule.start / 60 / 60) + cellWidth)
                let width = (program!.duration / 60 / 60) * 120
                cellAttributes.frame = CGRect(x: xPos - 70, y: yPos - 80, width: width, height: cellHeight)
            }else{
                if section == 0 && item == 0 {
            ...
            if let _ = program  {
              xPos = (cellWidth * (program!.schedule.start / 60 / 60) + cellWidth)
              let width = (program!.duration / 60 / 60) * 120
              cellAttributes.frame = CGRect(x: xPos - 80, y: yPos - 80, width: width, height: cellHeight)
            }
    ...
    self.contentSize = CGSize(width: contentWidth - 80, height: contentHeight - 80)
  }
