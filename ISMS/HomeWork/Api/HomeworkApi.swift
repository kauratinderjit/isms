...
class HomeworkApi {
    ...
    //MARK:- Homework Api
    func UpdateHomeworkData(url : String,parameters: [String : Any]?,completionResponse:  @escaping (HomeworkListModel) -> Void,completionnilResponse:  @escaping (String?) -> Void,complitionError: @escaping (Error?) -> Void){
        ...
        Alamofire.request(urlCmplete, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
            ...
            if response.result.isSuccess
            {
                ...
                if let responseData  = data as? [String : Any]
                {
                    ...
                    self.getHomeWorkJSON(data: responseData, completionResponse: { (responseModel) in
