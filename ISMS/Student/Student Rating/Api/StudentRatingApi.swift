...
class StudentRatingApi {
    
    static let sharedInstance = StudentRatingApi()
    
    //MARK:- Assign Subject to class Api
    func GetStudentList(url : String,parameters: [String : Any]?,completionResponse:  @escaping (StudentRatingListModel) -> Void,completionnilResponse:  @escaping (String?) -> Void,complitionError: @escaping (String?) -> Void){
        ...
        print("your entered parameter : \(parameters)")
        ...
        Alamofire.request(urlCmplete, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
            if response.result.isSuccess{
                ...
                if let responseData  = data as? [String : Any]{
                    print("your student rating response : \(responseData)")
                    self.StudentListJSON(data: responseData, completionResponse: { (response) in
    ...
    private func StudentListJSON(data: [String : Any],completionResponse:  @escaping (StudentRatingListModel) -> Void,completionError: @escaping (String?) -> Void)  {
        
        let assignSubjetcToClassData = StudentRatingListModel(JSON: data)
