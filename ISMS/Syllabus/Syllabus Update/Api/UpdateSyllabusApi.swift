...
class UpdateSyllabusApi {
    ...
    func AssignSubjectData(url : String,parameters: [String : Any]?,completionResponse:  @escaping (GetAssignSubjectModel) -> Void,completionnilResponse:  @escaping (String?) -> Void,complitionError: @escaping (Error?) -> Void){
           ...
           Alamofire.request(urlCmplete, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
               ...
               if response.result.isSuccess
               {
                   ...
                   {
                       print("your response update Data : \(responseData)")
                       self.getAssignSubjectJSON(data: responseData, completionResponse: { (responseModel) in
                           completionResponse(responseModel)
                       }, completionError: { (mapperError) in
                           completionnilResponse(mapperError)
                       })
       ...
       private func getAssignSubjectJSON(data: [String : Any],completionResponse:  @escaping (GetAssignSubjectModel) -> Void,completionError: @escaping (String?) -> Void)  {
           
           let ListData = GetAssignSubjectModel(JSON: data)
