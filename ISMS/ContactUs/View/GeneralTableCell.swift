           ...
           btnMinus.tag = indexPath.row
            ...
            if data?.count ?? 0 > 0{
                if data?.count == indexPath.row{
                    ...
                    if let phoneNum = (data?[indexPath.row] as! NSDictionary).value(forKey: "GenernalNumber") as? String{
                              txtFieldPhoneNum.text = phoneNum
                          }
                    if let email = (data?[indexPath.row] as! NSDictionary).value(forKey: "GenernalEmail") as? String{
