...
class RatingToTeacherApi {
    
    public static let sharedInstance = RatingToTeacherApi()
    
    
    //MARK:- Assign Subject to class Api
    func GetTeacherList(url : String,parameters: [String : Any]?,completionResponse:  @escaping (TeacherAllListModel) -> Void,completionnilResponse:  @escaping (String?) -> Void,complitionError: @escaping (String?) -> Void){
        ...
        Alamofire.request(urlCmplete, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
            if response.result.isSuccess{
                ...
                if let responseData  = data as? [String : Any]{
                    ...
                    self.TeacherListJSON(data: responseData, completionResponse: { (response) in
    ...
    private func TeacherListJSON(data: [String : Any],completionResponse:  @escaping (TeacherAllListModel) -> Void,completionError: @escaping (String?) -> Void)  {
        
        let assignSubjetcToClassData = TeacherAllListModel(JSON: data)
